...
void draw(char xpos, char ypos, char *text);
void drawLetter(char xpos, char ypos, char l);
...
void draw(char x, char y, char *message){
    int i = 0;
...
void drawLetter(char xpos, char ypos, char l) { 
    ...
    for (int i = 0; i < 5; i++) { 
        for (int j = 0; j < 8; j++) { 
            isOn = (ASCII[l-0x20][i]>>j) & 0b1;
            ssd1306_drawPixel(xpos + i, ypos + j, isOn);
        }
    }
}
