...
int main() {
    ...
    while (1) {
        ...
        for (i=0; i<NUM_DATA_PNTS; i++){
            ...
            // I wasn't present during the lecture, but I thought it would be
            // useful to translate acceleration in X,Y,Z into angles to tell
            // us the direction of movement - pitch and roll for planes (or
            // theta and phi angles of rotation)
            
            theta = atan(az[i] / ax[i]);
            phi = atan(az[i] / ay[i]);
